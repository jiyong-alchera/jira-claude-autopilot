...
| 경로 | 역할 |
|------|------|
| `run-jira-agent.sh` | 카드 1개를 plan/build 로 처리 (카드별 `repos/<repo>-<키>` 디렉토리, 병렬·멱등성 락) |
| `detect-cards.sh` | 대상 카드 탐지 (REST 우선, claude+MCP 폴백) |
| `lib-engine.sh` | LLM 엔진 추상화 — `claude`/`codex`/`gemini` 중 선택(프로젝트별·전역 기본값), 위 스크립트가 공유 |
...
- **카드 상태**: 단계(plan대기/답변대기/build대기/실패/완료) 표. 행을 펼치면 원문·코멘트 열람, **답변(인용 답글) 작성 + claude-answered 라벨**, **이 카드만 즉시 실행**, **엔진 실행 로그**(도구 호출/메시지/결과, 3초 갱신) 확인.
...
- `claude` (Claude Code CLI) 설치 + 로그인, `claude mcp add atlassian` 인증 (루프 내 Jira 작업용)
  - (선택) 다른 엔진을 쓰려면 해당 CLI 설치·로그인: `codex`(OpenAI) 또는 `gemini`(Google). 프로젝트 설정의 **LLM 엔진**에서 선택
...
`ENGINE`(기본 claude · codex/gemini 선택 가능), `MODEL`(비우면 엔진 기본 모델),
...
- `work.env`, `dashboard/config.json`, `dashboard/credentials.json`, `*.log`, `repos/`, `node_modules/`,
  `history.jsonl`, `loop-*.pid`, `agent-logs/` 는 `.gitignore` 로 커밋에서 제외됩니다.
