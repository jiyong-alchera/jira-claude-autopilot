...
# 개발 모드(핫리로드): 백엔드 자동 재시작 + 브라우저 라이브 리로드
npm run dev        # nodemon 으로 server.js/public 감시 후 재시작
```

> `npm start` 만 써도 프론트(`public/`) 수정 시 브라우저는 자동 새로고침됩니다(SSE 라이브 리로드).
> `npm run dev` 는 추가로 백엔드 코드(server.js) 변경 시 서버를 자동 재시작합니다(nodemon).
